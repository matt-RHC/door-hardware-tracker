...
export function applyCorrections(
  ...
  if (corrections.hardware_sets_corrections) {
    for (const corr of corrections.hardware_sets_corrections) {
      ...
      // Remove items (exact + case-insensitive only — NO substring matching
      // for removals, because "Hinge" would wrongly remove "Spring Hinge",
      // "Continuous Hinge", etc. Substring matching is safe for fixes/adds
      // where over-matching just means a missed correction, but for removals
      // it silently deletes data.)
