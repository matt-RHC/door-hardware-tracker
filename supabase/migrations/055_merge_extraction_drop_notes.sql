-- Migration 055: HOTFIX — drop openings.notes references from merge_extraction
--
-- Migration 054 dropped the `openings.notes` column but did not update the
-- `merge_extraction` RPC, which still references the column in three places:
--   1. UPDATE openings SET ... notes = v_staging.notes ... (v_hw_changed branch)
--   2. UPDATE openings SET ... notes = v_staging.notes ... (else branch, no hw change)
--   3. INSERT INTO openings (..., notes, ...) VALUES (..., v_staging.notes, ...)
--
-- The function's EXCEPTION WHEN OTHERS THEN handler swallows the error, so
-- merge_extraction returns {success: false, error: 'column "notes" of
-- relation "openings" does not exist'} for every promotion attempt. Sentry
-- doesn't see it; the failure surfaces as extraction_runs.status='failed'
-- and a 4xx-like JSON in the application response. Until this lands, every
-- attempt to promote a staged extraction to the live openings table fails.
--
-- This migration redefines merge_extraction with the three notes references
-- removed. The body is otherwise byte-identical to the live function (which
-- already has SET search_path TO 'public', 'pg_temp' applied at some point
-- after migration 037). Preserving SECURITY DEFINER and search_path so the
-- hardening posture stays consistent with migrations 052/053.
--
-- staging_openings.notes is intentionally left in place — write_staging_data
-- still extracts it from the JSON payload during the staging step, which is
-- harmless (just no consumer at promotion time anymore). A follow-up could
-- drop staging_openings.notes too, but that's lower priority.
...
COMMENT ON FUNCTION public.merge_extraction(uuid, uuid) IS
  ...
  'Migration 055 removed three references to openings.notes (column dropped in 054). '
  'SECURITY DEFINER + SET search_path = public, pg_temp per the 052/053 hardening pattern.';
