-- 034: Fix merge_extraction NULL array bug + issue_watches RLS gap
--
-- Bug 1: merge_extraction deactivates ALL openings when staging has no rows
--   v_staging_door_numbers is NULL when array_agg returns no rows, and
--   door_number != ALL(ARRAY[]::TEXT[]) is vacuously true, deactivating
--   every active opening in the project.
--
-- Bug 2: issue_watches RLS allows any authenticated user to watch issues
--   in projects they don't belong to, bypassing project membership.

-- ─── Fix 1: merge_extraction NULL-safe deactivation ────────────
...
BEGIN
  ...
  -- FIX: Guard against NULL v_staging_door_numbers which would deactivate ALL openings.
  -- array_agg() returns NULL (not empty array) when there are no staging rows.
...
-- ─── Fix 2: issue_watches RLS — require project membership ────
DROP POLICY IF EXISTS "Users can manage their own watches" ON issue_watches;
